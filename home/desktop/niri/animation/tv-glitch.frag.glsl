...
const float U_STRENGTH = 4.0;
const float U_SPEED    = 1.0;
...
// TV Spark consts
const float SPARK_BRIGHTNESS = 12.0;
const float SPARK_DELAY_EXP = 4.0;
...
vec4 glitch(vec3 coords_geo, vec3 size_geo, float progress) {
  ...
  float overshoot = max(1.0, progress);
  float undershoot = 1.0 + min(0.0, progress);
  coords_geo /= overshoot * undershoot;
  size_geo /= overshoot * undershoot;

  // Delay the TV squish to the second half of the animation
  float tvProgress = max(0.0, progress * 2.0 - 1.0);
  ...
  // Glitch
  float glitchProgress = progress;
  float time = glitchProgress * U_SPEED;
...
vec4 open_color(vec3 coords_geo, vec3 size_geo) {
  return glitch(coords_geo, size_geo, 1.0 - niri_progress);
}

vec4 close_color(vec3 coords_geo, vec3 size_geo) {
  return glitch(coords_geo, size_geo, niri_progress);
}